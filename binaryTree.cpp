...
class Solution {
  public:
    Node* markParents(Node* root,int target,map<Node*,Node*>& mpp){
    ...
    int findDistance(map<Node*,Node*> mpp,Node* target){
    ...
    int bfs(Node* root,int target){
        //find parents
        map<Node*,Node*> mpp;
        //find the address of target and map all parents to their node for transveresal
        Node* targetLoc = markParents(root,target,mpp);
        int maxi = findDistance(mpp,targetLoc);
        return maxi;
    }
    ...
    int minTime(Node* root, int target) 
    {
        ...
        //using bfs and striver approch
        //return bfs(root,target);

        //using gfg editorial
        return gfg(root,target);
